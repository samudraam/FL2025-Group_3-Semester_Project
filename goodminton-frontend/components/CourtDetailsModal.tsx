...
import { View, Text, StyleSheet, Modal, TouchableOpacity, ScrollView, Dimensions, Linking, Alert } from 'react-native';
...
{/* Header */} <Text style={styles.title}>{court.name}</Text> <Text style={styles.address}>{court.address}</Text>


      {/* 图片占位 */}
      <View style={styles.imageGallery}>
        <Text style={styles.placeholderText}>[Image gallery placeholder]</Text>
      </View>

      {/* 联系电话 & 距离占位 */}
      <View style={styles.infoRow}>
        <TouchableOpacity onPress={handleCallPress}>
          <Text style={styles.phoneText}>📞 Contact: {court.contact ?? " "}</Text>
        </TouchableOpacity>
        <Text> 📍 Distance: { distanceText ?? "Unknown"} </Text>
      </View>

      <Text style={styles.ratingText}>⭐ Rating: {court.rating?.toFixed(1) ?? "暂无"}</Text>
