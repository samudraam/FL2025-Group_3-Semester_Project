...
export default function FriendRequests({
  ...
  /**
   * Normalize API responses from different shapes
   */
  const getPendingRequests = useCallback(
    (response: unknown): FriendRequest[] => {
      if (Array.isArray(response)) {
        return response as FriendRequest[];
      }
      if (Array.isArray((response as any)?.pendingRequests)) {
        return (response as any)?.pendingRequests;
      }
      if (Array.isArray((response as any)?.data?.pendingRequests)) {
        return (response as any)?.data?.pendingRequests;
      }
      return [];
    },
    []
  ...
  /**
   * Fetch pending requests with optional refresh mode
   */
  const fetchFriendRequests = useCallback(
    async (isRefresh = false) => {
      try {
        ...
        const normalizedRequests = getPendingRequests(response);
        setFriendRequests(normalizedRequests);
        apiCache.set("friend-requests", normalizedRequests);
      } catch (error: any) {
        ...
        if (status === 429) {
          ...
          Alert.alert(
            "Error",
            error?.response?.data?.error || "Failed to load friend requests"
          );
        }
      } finally {
        setIsLoading(false);
  ...
  /**
   * Delay initial fetch to stagger API calls similar to GameRequests
   */
  useEffect(() => {
  ...
  useEffect(() => {
    console.log("[FriendRequests] mounted");
    return () => {
      console.log("[FriendRequests] unmounted");
    };
  }, []);
