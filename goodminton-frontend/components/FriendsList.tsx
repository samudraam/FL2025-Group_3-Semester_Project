...
export default function FriendsList({ onRefresh }: FriendsListProps) {
  ...
  const [teamAPlayers, setTeamAPlayers] = useState<Friend[]>(currentUserPlayer ? [currentUserPlayer] : []);
   ...
   */
  const handleScoreChange = (setKey: ScoreKey, team: TeamKey, value: string) => {
    setScores((prev) => ({
      ...prev,
      [setKey]: {
        ...prev[setKey],
    ...
    if (type === 'singles') {
      ...
      if (currentUserPlayer) {
        setTeamAPlayers((prev) => {
          const withoutCurrent = prev.filter((player) => player.id !== currentUserPlayer.id);
  ...
  const handleSearchPlayers = async (team: TeamKey) => {
    const query = (team === 'teamA' ? teamASearchQuery : teamBSearchQuery).trim();
    if (!query) {
      Alert.alert('Enter an email to search');
      return;
    }

    const setSearching = team === 'teamA' ? setTeamASearching : setTeamBSearching;
    const setResults = team === 'teamA' ? setTeamASearchResults : setTeamBSearchResults;
    ...
    try {
      ...
      if (uniqueCandidates.length === 0) {
        Alert.alert('No results', 'No matching players found or they are already added.');
      } else {
        setResults(uniqueCandidates);
      }
    } catch (error: any) {
      const message = error?.response?.data?.error || 'Search failed. Please try again.';
  ...
  const handleAddPlayerToTeam = (team: TeamKey, player: Friend) => {
    ...
    if (currentUserPlayer && player.id === currentUserPlayer.id && team === 'teamB') {
      Alert.alert('Invalid selection', 'You cannot add yourself to Team 2.');
      return;
    }

    const existsInTeam =
      (team === 'teamA' ? teamAPlayers : teamBPlayers).some((member) => member.id === player.id);
    const existsInOpposingTeam =
      (team === 'teamA' ? teamBPlayers : teamAPlayers).some((member) => member.id === player.id);
    ...
    if (team === 'teamA') {
      ...
      if (teamBPlayers.length >= limit) {
        Alert.alert('Team full', `Team 2 already has ${limit} player${limit === 1 ? '' : 's'}.`);
        return;
      }
  ...
  const handleRemovePlayerFromTeam = (team: TeamKey, playerId: string) => {
    if (team === 'teamA') {
      ...
      setTeamAPlayers((prev) => prev.filter((player) => player.id !== playerId));
    } else {
      setTeamBPlayers((prev) => prev.filter((player) => player.id !== playerId));
    }
  };
  ...
  const handleCreateGameSubmit = async () => {
    try {
      ...
      for (const key of scoreKeys) {
        ...
        if (!teamAScoreRaw || !teamBScoreRaw) {
          Alert.alert('Invalid Input', `${setLabels[key]} must include scores for both teams.`);
          return;
        }
        ...
        if (Number.isNaN(teamAScore) || Number.isNaN(teamBScore)) {
          Alert.alert('Invalid Input', `${setLabels[key]} scores must be numbers.`);
          return;
        }
      ...
      if (scoreSets.length === 0) {
        Alert.alert('Invalid Input', 'Please provide at least one completed set.');
        return;
      }
      ...
      if (gameType === 'singles') {
        ...
        const winnerId = winner === 'teamA' ? currentUserPlayer.id : opponent.id;
        ...
        const teammate = teamAPlayers.find((player) => currentUserPlayer && player.id !== currentUserPlayer.id);
        if (!teammate?.id) {
        ...
        if (!opponent1?.id || !opponent2?.id) {
          Alert.alert('Invalid Input', 'Opponents must have valid player records.');
          return;
        }
          ...
          <View style={styles.modalContent}>
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <View style={styles.teamSection}>
                ...
                {teamAPlayers.length === 0 ? (
                  <Text style={styles.teamEmptyText}>Add players to Team 1.</Text>
                ) : (
                  teamAPlayers.map((player) => {
                    const canRemove = !currentUserPlayer || player.id !== currentUserPlayer.id;
                    return (
                      <View key={player.id} style={styles.teamPlayerRow}>
                        <Text style={styles.teamPlayerName}>{getFriendDisplayName(player)}</Text>
                        {canRemove && (
                          <Pressable
                            style={styles.removePlayerButton}
                            onPress={() => handleRemovePlayerFromTeam('teamA', player.id)}
                          >
                    ...
                    <View style={styles.teamSearchRow}>
                      ...
                      >
                        {teamASearching ? (
                          ...
                          <Text style={styles.teamSearchButtonText}>Search</Text>
                        )}
                      </Pressable>
                    </View>
                    {teamASearchResults.map((player) => (
                      <View key={player.id} style={styles.searchResultRow}>
                        <Text style={styles.searchResultName}>{getFriendDisplayName(player)}</Text>
                        <Pressable
                  ...
                  teamBPlayers.map((player) => (
                    <View key={player.id} style={styles.teamPlayerRow}>
                      ...
                      {gameType === 'doubles' && (
                        <Pressable
                          style={styles.removePlayerButton}
                          onPress={() => handleRemovePlayerFromTeam('teamB', player.id)}
                        >
                          <Text style={styles.removePlayerText}>Remove</Text>
                        </Pressable>
                      )}
                ...
                <View style={styles.scoreInputContainer}>
                  <View style={styles.scoreInputWrapper}>
                    ...
                    <TextInput
                      ...
                      onChangeText={(value) => handleScoreChange('set1', 'teamA', value)}
                      placeholder="0"
                  ...
                  <View style={styles.scoreInputWrapper}>
                    ...
                    <TextInput
                      ...
                      onChangeText={(value) => handleScoreChange('set1', 'teamB', value)}
                      placeholder="0"
                      keyboardType="numeric"
                      maxLength={2}
                    />
                ...
                <View style={styles.scoreInputContainer}>
                  <View style={styles.scoreInputWrapper}>
                    ...
                    <TextInput
                      ...
                      onChangeText={(value) => handleScoreChange('set2', 'teamA', value)}
                      placeholder="0"
                      keyboardType="numeric"
                      maxLength={2}
                    />
                  ...
                  <View style={styles.scoreInputWrapper}>
                    ...
                    <TextInput
                      ...
                      onChangeText={(value) => handleScoreChange('set2', 'teamB', value)}
                      placeholder="0"
                      keyboardType="numeric"
                      maxLength={2}
                    />
                ...
                <View style={styles.scoreInputContainer}>
                  <View style={styles.scoreInputWrapper}>
                    ...
                    <TextInput
                      ...
                      onChangeText={(value) => handleScoreChange('set3', 'teamA', value)}
                      placeholder="0"
                      keyboardType="numeric"
                      maxLength={2}
                    />
                  ...
                  <View style={styles.scoreInputWrapper}>
                    ...
                    <TextInput
                      ...
                      onChangeText={(value) => handleScoreChange('set3', 'teamB', value)}
                      placeholder="0"
                      keyboardType="numeric"
                      maxLength={2}
                    />
                ...
                <View style={styles.winnerButtons}>
                  ...
                  >
                    ...
                    <Text
                      style={[
                        ...
                        winner === 'teamA' && styles.winnerButtonSublabelSelected,
                      ]}
                    >
                      {teamADisplayName}
                    </Text>
                  ...
                  >
                    ...
                    <Text
                      style={[
                        styles.winnerButtonSublabel,
                        winner === 'teamB' && styles.winnerButtonSublabelSelected,
                      ]}
                    >
                      {teamBDisplayName}
                    </Text>
