...
type EditCommunityModalProps = {
  ...
  onClose: () => void;
  onUpdated?: (community: CommunitySummary) => void;
  onDeleted?: () => void;
};
...
type AdminRosterEntry = AdminCandidate & {
  role: "owner" | "admin";
};
...
/**
 * Modal for editing existing badminton communities.
 */
const EditCommunityModal = ({
  ...
  const communityIdentifier = community?.id || community?.slug || "";
  ...
  const [adminSearchResults, setAdminSearchResults] = useState<
    AdminCandidate[]
  >([]);
  const [adminRoster, setAdminRoster] = useState<AdminRosterEntry[]>([]);
  ...
  }, [community, communityName, communitySlug, isSubmitting, isUploadingCover]);

  const previewUri = useMemo(() => {
    return coverPreviewUri || coverImageUrl;
  }, [coverPreviewUri, coverImageUrl]);

  const hydrateFromCommunity = useCallback(() => {
    if (!community) {
    ...
    setCommunityStatus(community.visibility || "private");
    setCommunityName(community.name || "");
    setCommunitySlug(community.slug || "");
    setDescription(community.description || "");
    const coverUri = community.coverImageUrl || "";
    setCoverImageUrl(coverUri);
    setCoverPreviewUri(coverUri);
  ...
  useEffect(() => {
    if (visible) {
      hydrateFromCommunity();
      resetTransientState();
    }
  }, [visible, hydrateFromCommunity, resetTransientState]);

  const loadAdminRoster = useCallback(async () => {
    if (!canManageAdmins || !communityIdentifier) {
      setAdminRoster([]);
    ...
    try {
      const response = await fetchWithRetry(
        () => communitiesAPI.listAdmins(communityIdentifier),
        { skipCache: true }
      );
      const roster: AdminRosterEntry[] = Array.isArray(response?.admins)
        ? response.admins
            .filter((entry: any) => entry?.id)
            .map((entry: any) => ({
              id: entry.id,
              displayName: entry.displayName || "Community member",
              email: entry.email,
      ...
      console.error("Load admin roster failed", error);
      Alert.alert(
  ...
  }, [canManageAdmins, communityIdentifier]);

  useEffect(() => {
    if (visible) {
      loadAdminRoster();
    }
  }, [visible, loadAdminRoster]);
  ...
  const handleCoverImageSelection = useCallback(
    async (source: "camera" | "library") => {
      ...
      try {
        ...
        Alert.alert(
          "Upload failed",
          error?.message || "We couldn't upload that image. Please try again."
        );
      } finally {
  ...
  const handleSearchAdmins = useCallback(async () => {
    ...
    if (!canManageAdmins) {
      Alert.alert("Insufficient permissions", "You cannot edit admins.");
      return;
    }
    ...
    try {
      const response = await fetchWithRetry(
        () => usersAPI.search(adminSearchInput.trim()),
        { skipCache: true }
      );
      ...
      const uniqueCandidates = candidates.filter(
        (candidate, index, arr) =>
          ...
          !adminRoster.some((admin) => admin.id === candidate.id)
      );
      if (uniqueCandidates.length === 0) {
        Alert.alert(
          "No matches",
          "No eligible users found or they are already admins."
        );
        return;
      }
  ...
  }, [adminSearchInput, adminRoster, canManageAdmins, normalizeToCandidate]);
  ...
  const handleAddAdmin = useCallback((candidate: AdminCandidate) => {
    setAdminRoster((prev) => {
      ...
      return [...prev, { ...candidate, role: "admin" }];
    });
    setAdminSearchInput("");
    setAdminSearchResults([]);
  }, []);
  ...
  const handleRemoveAdmin = useCallback((candidateId: string) => {
    setAdminRoster((prev) => prev.filter((admin) => admin.id !== candidateId));
  }, []);

  const syncAdmins = useCallback(
    async (identifier: string) => {
      if (!canManageAdmins || !identifier) {
        return { promoted: 0, demoted: 0, failed: 0 };
      }
      ...
      const initialSet = initialAdminIdsRef.current;
      const toPromote = Array.from(currentSet).filter(
        (id) => id && !initialSet.has(id)
      );
      const toDemote = Array.from(initialSet).filter(
        (id) => id && !currentSet.has(id)
      );
      let promoted = 0;
      let demoted = 0;
      let failed = 0;

      for (const adminId of toPromote) {
        try {
          const response = await fetchWithRetry(
            () => communitiesAPI.promoteAdmin(identifier, adminId),
            { skipCache: true }
          );
          if (response.success) {
            promoted += 1;
      ...
      for (const adminId of toDemote) {
        try {
          const response = await fetchWithRetry(
            () => communitiesAPI.demoteAdmin(identifier, adminId),
            { skipCache: true }
          );
          if (response.success) {
            demoted += 1;
      ...
      initialAdminIdsRef.current = new Set(currentSet);
      return { promoted, demoted, failed };
    },
    [adminRoster, canManageAdmins]
  );

  const handleSubmit = useCallback(async () => {
    if (!community || !communityIdentifier) {
      Alert.alert("Community unavailable", "Please try again later.");
      return;
    }
    ...
    const trimmedSlug = communitySlug.trim();
    if (!trimmedSlug) {
      Alert.alert("Missing slug", "Provide a slug for this community.");
      return;
    }
    ...
    try {
      ...
      const response = await fetchWithRetry(
        () => communitiesAPI.update(communityIdentifier, payload),
        { skipCache: true }
      );
      if (!response.success || !response.community) {
        throw new Error(response.error || "Failed to update community.");
      }
      const adminSummary = await syncAdmins(communityIdentifier);
      if (canManageAdmins) {
        await loadAdminRoster();
      }
      const adminMessage = canManageAdmins
        ? `\nAdmins -> Added: ${adminSummary.promoted}, Removed: ${adminSummary.demoted}, Failed: ${adminSummary.failed}.`
        : "";
      Alert.alert(
        "Community updated",
        `Your changes have been saved.${adminMessage}`,
        [{ text: "Great", onPress: handleClose }]
      );
      onUpdated?.(response.community);
    } catch (error: any) {
  ...
  const confirmDeleteCommunity = useCallback(() => {
    if (!isOwner || !communityIdentifier) {
      Alert.alert("Unavailable", "Only the owner can delete this community.");
      return;
    }
    Alert.alert(
      "Delete community",
      "This action permanently removes the community, posts, and events. Continue?",
      [
        { text: "Cancel", style: "cancel" },
        {
          ...
          onPress: async () => {
            ...
            try {
              const response = await fetchWithRetry(
                () => communitiesAPI.delete(communityIdentifier),
                { skipCache: true }
              );
              if (!response.success) {
                throw new Error(
                  response.error || "Unable to delete community."
                );
              }
              Alert.alert("Community deleted", "This group has been removed.");
              onDeleted?.();
              handleClose();
            } catch (error: any) {
  ...
  }, [communityIdentifier, handleClose, isOwner, onDeleted]);

  const renderAdminChip = (admin: AdminRosterEntry) => (
    <View key={admin.id} style={styles.adminChip}>
      <Text style={styles.adminChipText}>{admin.displayName}</Text>
      <Text style={styles.adminChipRole}>
      ...
      >
        <View style={styles.overlay}>
          <View style={styles.modalCard}>
            <Text style={styles.title}>Community unavailable</Text>
            <Text style={styles.subtitle}>
              We could not load this community. Please close and try again.
            </Text>
      ...
      <View style={styles.overlay}>
        <View style={styles.modalCard}>
          ...
          >
            ...
            <Text style={styles.subtitle}>
              Update details for {community.name}
            </Text>
              ...
              <View style={styles.section}>
                ...
                {isRosterLoading ? (
                  <View style={styles.rosterLoadingRow}>
                    ...
                    <Text style={styles.rosterLoadingText}>
                      Loading admins...
                    </Text>
                  </View>
                ) : adminRoster.length > 0 ? (
                  <View style={styles.selectedAdminsContainer}>
                    {adminRoster.map(renderAdminChip)}
                  </View>
                ) : (
                  <Text style={styles.helperText}>
                    Add trusted members to help moderate this community.
                  </Text>
                )}
                  ...
                  <View key={candidate.id} style={styles.searchResultRow}>
                    <View style={styles.searchCandidateInfo}>
                      ...
                      <View style={styles.searchCandidateText}>
                        ...
                        {candidate.email ? (
                          <Text style={styles.searchCandidateEmail}>
                            {candidate.email}
                          </Text>
                        ) : null}
                      </View>
            ...
            {isOwner ? (
              <Pressable
                style={[
                  styles.deleteButton,
                  isDeleting && styles.submitButtonDisabled,
                ]}
...
export default React.memo(EditCommunityModal);
